...
public class ZomMapView extends android.support.v7.widget.AppCompatImageView {
    ...
    public void onMapClick() {
        ...
        rectF = getImageBound(null);
        bottom = rectF.bottom;
        left = rectF.left;
        right = rectF.right;
        top = rectF.top;

        float imgWidth = (right - left);
        float imgHeight = (bottom - top);

        //Log.e("imgWidth", "imgWidth：" + imgWidth + " imgHeight：" + imgHeight);

        float zoom = imgWidth / width;

        float offsetX = Math.abs((int) left);
        float offsetY = Math.abs((int) top);

        //Log.e("rectF", "width：" + width + " height：" + height);
        //Log.e("rectF", "放大倍数：" + zoom + " offsetX：" + offsetX + " offsetY：" + offsetY + " pointX：" + pointX + " pointY：" + pointY);

        float pointOffsetX = (pointX + offsetX) / zoom;
        float pointOffsetY = (pointY + offsetY) / zoom;

        float persentX = pointOffsetX / width;
        float persentY = pointOffsetY / height;

        if(chooseListener!=null){
            chooseListener.onChoose(persentX,persentY);
        }
